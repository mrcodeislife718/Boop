...
export class RailFabric {
  ...
  register(capability: RailCapability): RailCapability {
    ...
    if (capability.estimatedCostBps < 0 || capability.fixedFeeMinor < 0n || capability.expectedLatencyMs < 0) {
      throw new Error('Rail costs and latency must be non-negative');
    }
    if (!Number.isFinite(capability.reliability) || capability.reliability < 0 || capability.reliability > 1) {
      throw new Error('Rail reliability must be between 0 and 1');
    }
